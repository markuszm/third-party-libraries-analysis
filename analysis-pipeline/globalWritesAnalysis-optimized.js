...
(function(sandbox) {
    function GlobalWritesAnalysis() {
        ...
        var reachablePathCache = new Map();

        this.putField = function(iid, base, offset, val, isComputed, isOpAssign) {
            alreadyVisited.clear();

            var cachedReachablePath = null;

            reachablePathCache.forEach((value, key) => {
                const obj = value;
                if (obj && Object.getOwnPropertyNames(obj).includes(offset)) {
                    if((myType(base) === 'String' || myType(base) === 'Number') && myType(obj) === 'Function') {
                        return;
                    }
                    cachedReachablePath = key;
                }
            })

            var reachablePath;

            if (cachedReachablePath) {
                reachablePath = cachedReachablePath;
            } else {
                reachablePath = findReachableAccessPath(base, alreadyVisited, window, 'window');
            }
            ...
            if (reachablePath) {
                if (reachablePath !== 'window') {
                    ...
                    reachablePathCache.set(reachablePath, getObjectForPath(reachablePath));
                }
            }
        ...
        function getObjectForPath(path) {
            ...
            for (const segment of segments) {
                try {
                    start = Reflect.get(start, segment);
                } catch (err) {
                    return null;
                }
            }
